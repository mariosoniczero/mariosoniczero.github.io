...
TeamDropdownRoom=function(_PSRoom){_inheritsLoose(TeamDropdownRoom,_PSRoom);




function TeamDropdownRoom(options){var _this;
_this=_PSRoom.call(this,options)||this;_this.classType='user';
_this.userid=_this.id.slice(5);
_this.isSelf=_this.userid===PS.user.userid;
_this.name=options.username||_this.userid;
if(/[a-zA-Z0-9]/.test(_this.name.charAt(0)))_this.name=' '+_this.name;
PS.send("|/cmd userdetails "+_this.userid);return _this;
}return TeamDropdownRoom;}(PSRoom);var


TeamDropdownPanel=function(_PSRoomPanel){_inheritsLoose(TeamDropdownPanel,_PSRoomPanel);function TeamDropdownPanel(){var _this2;for(var _len=arguments.length,args=new Array(_len),_key=0;_key<_len;_key++){args[_key]=arguments[_key];}_this2=_PSRoomPanel.call.apply(_PSRoomPanel,[this].concat(args))||this;_this2.
gen='';_this2.
format=null;_this2.
...
_this2.format=target.name==='format'&&target.value||'';
_this2.gen=target.name==='gen'&&target.value||'';
_this2.forceUpdate();
};_this2.
click=function(e){
...
_this2.props.room.parentElem.value=target.value;
PS.closePopup();
};return _this2;}var _proto=TeamDropdownPanel.prototype;_proto.getTeams=function getTeams(){var _this3=this;if(!this.format&&!this.gen)return PS.teams.list;return PS.teams.list.filter(function(team){if(_this3.gen&&!team.format.startsWith(_this3.gen))return false;if(_this3.format&&team.format!==_this3.format)return false;return true;});};_proto.
...
preact.h("button",{"class":'button'+(gen===_this4.gen?' disabled':''),onClick:_this4.setFormat,name:"gen",value:gen},
...
};return TeamDropdownPanel;}(PSRoomPanel);
