...
if(clientPokemon.volatiles['hyperion']){
stats.atk=Math.floor(stats.atk*1.5);
...
if(ability==='quickfeet'||ability==='momentum'){
speedModifiers.push(1.5);
}
}
...
if(clientPokemon!=null&&clientPokemon.volatiles['pristinearmor']){
stats.def=Math.floor(stats.def*1.25);
stats.spd=Math.floor(stats.spd*1.25);
}
if(ability==='livingcitadel'){
stats.spd*=2;
}
...
if(item==='choicespecs'&&!(clientPokemon!=null&&clientPokemon.volatiles['dynamax'])||ability==='heavyartillery'){
stats.spa=Math.floor(stats.spa*1.5);
}
...
if(ability==='bushido'){
var statusCount=0;for(var _i34=0,_pokemon$moveSlots2=
pokemon.moveSlots;_i34<_pokemon$moveSlots2.length;_i34++){var moveSlot=_pokemon$moveSlots2[_i34];
...
if(this.battle.abilityActive('Deadly Precision')){
if(ability!=='deadlyprecision'){
...
stats.spa=Math.floor(stats.spa*1.5);
}for(var _i36=0,_Dex$statNamesExceptH8=
Dex.statNamesExceptHP;_i36<_Dex$statNamesExceptH8.length;_i36++){var _statName3=_Dex$statNamesExceptH8[_i36];
...
var chainedSpeedModifier=1;for(var _i38=0;_i38<
speedModifiers.length;_i38++){var modifier=speedModifiers[_i38];
...
if(!short){
var hasModifiedStat=false;for(var _i40=0,_Dex$statNamesExceptH10=
Dex.statNamesExceptHP;_i40<_Dex$statNamesExceptH10.length;_i40++){var statName=_Dex$statNamesExceptH10[_i40];
...
buf+='<p>';
}for(var _i42=0,_Dex$statNamesExceptH12=

Dex.statNamesExceptHP;_i42<_Dex$statNamesExceptH12.length;_i42++){var _statName4=_Dex$statNamesExceptH12[_i42];
...
ppUsed=function ppUsed(move,pokemon){for(var _i44=0,_pokemon$moveTrack2=
pokemon.moveTrack;_i44<_pokemon$moveTrack2.length;_i44++){var _ref5=_pokemon$moveTrack2[_i44];var moveName=_ref5[0];var _ppUsed=_ref5[1];
...
if(move.id==='triattack'&&value.abilityModify(0,'Analyzer')&&['Fire','Ice','Electric'].includes(pokemon.teraType)){
moveType=pokemon.teraType;
}
...
if(value.abilityModify(0,'Conflagrate'))moveType='Fire';
if(value.abilityModify(0,'Infectate'))moveType='Bug';
}
...
if((attackType==='Ground'||attackType==='Water')&&abilityid==='sunkensky')return 0;
if(attackType==='Dragon'&&abilityid==='dragonhunter')return 0;
...
var factor=1;
if(!otherFactor&&(targetAbility==="Levitate"||targetAbility==="Sunken Sky")){
otherFactor=1;
if(!target.isGrounded()&&move.id!=='thousandarrows'&&!hardcoreMode){
factor=0;
}
}for(var _i46=0;_i46<
targetTypes.length;_i46++){var _tType$damageTaken,_tType$damageTaken2,_tType$damageTaken3,_tType$damageTaken4;var targetType=targetTypes[_i46];
...
if(this.battle.hardcoreMode){
var _tags='';for(var _i48=0,_foeActive6=
foeActive;_i48<_foeActive6.length;_i48++){var possibleTarget=_foeActive6[_i48];
if(!possibleTarget)continue;
var effectiveness=this.getMoveEffectiveness(pokemon,move,moveType,category,possibleTarget);
...
var tags="\xD7";for(var _i50=0,_foeActive8=
foeActive;_i50<_foeActive8.length;_i50++){var _possibleTarget=_foeActive8[_i50];
...
if(move.category==='Status'){
value.abilityModify(0,'Spectral Flames');
}
...
}for(var _i52=0,_pokemon$side$active2=

pokemon.side.active;_i52<_pokemon$side$active2.length;_i52++){var active=_pokemon$side$active2[_i52];
...
var chain=4096;for(var _i54=0;_i54<
accuracyModifiers.length;_i54++){var mod=accuracyModifiers[_i54];
if(mod!==4096){
...
var boostCount=0;for(var _i56=0,_Object$values2=
Object.values(target.boosts);_i56<_Object$values2.length;_i56++){var boost=_Object$values2[_i56];
...
var _boostCount=0;for(var _i58=0,_Object$values4=
Object.values(pokemon.boosts);_i58<_Object$values4.length;_i58++){var _boost=_Object$values4[_i58];
...
value.abilityModify(1.1,"Lunar Power");
}
if(move.type==='Dragon'){
value.abilityModify(1.3,"Dragonhunter");
}

if(pokemon.status==='brn'&&move.category==='Special'){
value.abilityModify(1.5,"Flare Boost");
value.abilityModify(1.5,"Hindenburg");
}
if(move.flags['punch']){
value.abilityModify(1.2,'Iron Fist');
value.abilityModify(1.5,'Magnetic Fists');
}
if(move.flags['pulse']){
...
value.abilityModify(1.3,"Spectral Battery");
}
if(value.value<=60){
...
value.abilityModify(1.2,"Conflagrate");
value.abilityModify(1.2,"Infectate");
}
if(this.battle.gen>6){
value.abilityModify(1.2,"Normalize");
}
}
if(move.id==='triattack'&&['Fire','Ice','Electric'].includes(pokemon.teraType)){
value.abilityModify(1.2,'Analyzer');
}
...
if(move.category!=='Status'){
var auraBoosted='';
var auraBroken=false;for(var _i60=0,_pokemon$side$active4=
pokemon.side.active;_i60<_pokemon$side$active4.length;_i60++){var ally=_pokemon$side$active4[_i60];
if(!ally||ally.fainted)continue;
var allyAbility=this.getAllyAbility(ally);
...
}for(var _i62=0,_pokemon$side$foe$act2=
pokemon.side.foe.active;_i62<_pokemon$side$foe$act2.length;_i62++){var foe=_pokemon$side$foe$act2[_i62];
...
var _boostCount2=0;for(var _i64=0,_Object$values6=
Object.values(pokemon.boosts);_i64<_Object$values6.length;_i64++){var _boost2=_Object$values6[_i64];
...
for(var _i65=1;_i65<=5&&_i65<=pokemon.side.faintCounter;_i65++){
if(pokemon.volatiles["fallen"+_i65]){
value.abilityModify([1,1.15,1.3,1.45,1.6,1.75][_i65],"Hardcore Hustle");
}
}
...
for(var _i66=1;_i66<=5&&_i66<=pokemon.side.faintCounter;_i66++){
if(pokemon.volatiles["fallen"+_i66]){
value.abilityModify(1+0.05*_i66,"The Eminence in the Shadow");
}
}
}
...
if(!types)types=this.getPokemonTypes(pokemon);for(var _i68=0,_types2=
types;_i68<_types2.length;_i68++){var curType=_types2[_i68];
...
if(fusionSpecies.exists&&fusionSpecies.name!==species.name){for(var _i70=0,_Object$values8=
Object.values(fusionSpecies.abilities);_i70<_Object$values8.length;_i70++){var newAbility=_Object$values8[_i70];
