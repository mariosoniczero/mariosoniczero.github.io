...
function getFlingPower(item) {
    ...
    if (item === 'Iron Ball')
        return 130;
    if (['Hard Stone', 'Room Service'].indexOf(item) !== -1)
        return 100;
    ...
    if (FLING_80.has(item))
        return 80;
    if (['Poison Barb', 'Dragon Fang'].indexOf(item) !== -1)
        return 70;
    if (FLING_60.has(item))
        return 60;
    if (['Eject Pack', 'Sharp Beak'].indexOf(item) !== -1)
        return 50;
    if (['Icy Rock', 'Eviolite'].indexOf(item) !== -1)
        return 40;
