...
function getRecovery(gen, attacker, defender, move, damage, notation) {
    ...
    var recovery = [0, 0];
    var text = '';
    var ignoresShellBell = gen === 3 && (move.name === 'Doom Desire' || move.name === 'Future Sight');
...
function getKOChance(gen, attacker, defender, move, field, damage, err) {
    ...
    if ((move.usedTimes === 1 && move.metronomeCount === 1) || move.isZ) {
        ...
        var chance = computeKOChance(damage, defender.maxHP() - hazards.damage, eot.damage, move.usedTimes || 1, move.usedTimes || 1, defender.maxHP(), toxicCounter);
        if (chance === 1) {
            return {
                chance: chance,
                n: move.usedTimes,
        ...
        if (predictTotal(damage[0], eot.damage, move.usedTimes, move.usedTimes, toxicCounter, defender.maxHP()) >=
            defender.curHP - hazards.damage) {
            return {
                chance: 1,
                n: move.usedTimes,
        ...
        else if (predictTotal(damage[damage.length - 1], eot.damage, move.usedTimes, move.usedTimes, toxicCounter, defender.maxHP()) >=
            defender.curHP - hazards.damage) {
...
function getHazards(gen, defender, defenderSide) {
    ...
    if (defenderSide.isSR && !defender.hasAbility('Magic Guard', 'Mountaineer')) {
        var effectiveness = types_1.TYPE_CHART[gen]['Rock'][defender.type1] *
            (defender.type2 ? types_1.TYPE_CHART[gen]['Rock'][defender.type2] : 1);
        damage += Math.floor((effectiveness * defender.maxHP()) / 8);
        texts.push('Stealth Rock');
    }
    if (defenderSide.steelsurge && !defender.hasAbility('Magic Guard', 'Mountaineer')) {
        var effectiveness = types_1.TYPE_CHART[gen]['Steel'][defender.type1] *
            (defender.type2 ? types_1.TYPE_CHART[gen]['Steel'][defender.type2] : 1);
        damage += Math.floor((effectiveness * defender.maxHP()) / 8);
