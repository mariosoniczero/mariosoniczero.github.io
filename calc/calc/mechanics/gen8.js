...
exports.calculateSS = makeCalculate(SS);
function calculateModern(gen, attacker, defender, move, field) {
    ...
    util_1.checkTerrify(attacker, defender);
    util_1.checkTerrify(defender, attacker);
    ...
    if (isAerilate || isPixilate || isRefrigerate || isGalvanize || isNormalize || isConflagrate || isInfectate) {
    ...
    var attackStat = move.category === 'Special' ? 'spa' : move.name === 'Body Press' ? 'atk' : 'def';
    ...
    if (util_1.isGrounded(attacker, field)) {
        if (field.terrain === 'Electric' && move.type === 'Electric') {
            baseDamage = util_1.pokeRound((baseDamage * 0x14cd) / 0x1000);
            description.terrain = field.terrain;
        }
