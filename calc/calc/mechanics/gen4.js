...
function calculateDPP(attacker, defender, move, field) {
    ...
    attacker.stats.spe = util_1.getFinalSpeed(DPP, attacker, field, field.attackerSide);
    defender.stats.spe = util_1.getFinalSpeed(DPP, defender, field, field.defenderSide);
    var description = {
    ...
    var damage = [];
    var result = new result_1.Result(DPP, attacker, defender, move, field, damage, description);
    if (move.bp === 0) {
        damage.push(0);
        return result;
    }
    ...
    if (move.name === 'Weather Ball') {
        ...
        var gift = items_1.getNaturalGift(DPP, attacker.item);
        move.type = gift.t;
        move.bp = gift.p;
    ...
    var typeEffect1 = util_1.getMoveEffectiveness(DPP, move, defender.type1, attacker.hasAbility('Scrappy') || field.defenderSide.isForesight, field.isGravity);
    var typeEffect2 = defender.type2
        ? util_1.getMoveEffectiveness(DPP, move, defender.type2, attacker.hasAbility('Scrappy') || field.defenderSide.isForesight, field.isGravity)
        : 1;
    ...
    switch (move.name) {
        ...
        case 'Payback':
            ...
            break;
        case 'Punishment':
            var boostCount = util_1.countBoosts(DPP, defender.boosts);
            if (boostCount > 0) {
    ...
    else if (items_1.getItemBoostType(attacker.item) === move.type ||
