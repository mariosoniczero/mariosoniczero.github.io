...
function calculateSMSS(gen, attacker, defender, move, field) {
    ...
    util_2.checkWonderRoom(attacker, field.isWonderRoom);
    util_2.checkWonderRoom(defender, field.isWonderRoom);
    ...
    util_2.checkTerrify(gen, attacker, defender);
    util_2.checkTerrify(gen, defender, attacker);
    ...
    if (move.named('Weather Ball')) {
        ...
        (move.named('Terrain Pulse') && util_2.isGrounded(attacker, field))) {
        type =
    ...
    if (!move.isZ && !noTypeChange) {
        ...
        if (isGalvanize || isPixilate || isRefrigerate || isAerilate || isNormalize || isConflagrate || isInfectate) {
            desc.attackerAbility = attacker.ability;
    ...
    if (attacker.hasAbility('Parental Bond (Child)')) {
        baseDamage = util_2.pokeRound(util_2.OF32(baseDamage * 1024) / 4096);
    }
...
function calculateBPModsSMSS(gen, attacker, defender, move, field, desc, basePower, hasAteAbilityTypeChange, turnOrder) {
    ...
    else if (move.named('Expanding Force') && util_2.isGrounded(attacker, field) && field.hasTerrain('Psychic')) {
    ...
    if (attacker.hasItem(move.type + " Gem")) {
        ...
        attacker.item && move.hasType(items_1.getItemBoostType(attacker.item))) {
        bpMods.push(4915);
        desc.attackerItem = attacker.item;
    }
...
function calculateDefenseSMSS(gen, attacker, defender, move, field, desc, isCritical) {
    ...
    return util_2.OF16(Math.max(1, util_2.pokeRound((defense * util_2.chainMods(dfMods)) / 4096)));
}
