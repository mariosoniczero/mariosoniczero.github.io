...
function getFinalSpeed(gen, pokemon, field, side) {
    ...
    if ((pokemon.hasAbility('Unburden') && pokemon.abilityOn) ||
        ...
        (pokemon.hasAbility('Weather Rush') && (weather.includes('Rain') || weather === 'Hail' || weather === 'Sand'))) {
        speed *= 2;
    }
...
function checkTerrify(gen, source, target) {
...
function checkMultihitBoost(gen, attacker, defender, move, field, desc, usedWhiteHerb) {
    ...
    if (defender.hasAbility('Stamina')) {
        if (attacker.hasAbility('Unaware') || attacker.hasAbility('Brainless')) {
